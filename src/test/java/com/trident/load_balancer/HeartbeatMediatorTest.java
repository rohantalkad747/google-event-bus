...
class HeartbeatMediatorTest {
    private final HeartbeatMediator heartbeatMediator = new HeartbeatMediator(ClusterExamples.CLUSTER_HALF_SECOND_HB);
    private final Long lh8080ExampleHbTs = HeartbeatMonitorTest.HeartbeatExamples.VALID.getTimeEpochMs();


    @Test
    void testNewHbRecorded() {
        heartbeatMediator.onNewHb(NodeExamples.LOCAL_HOST_8080, HeartbeatMonitorTest.HeartbeatExamples.VALID);
        expectDefaultTs();
    }
    ...
    void testOutOfSynchHbNotOverwritten() {
        ...
        assertThrows(OutOfSyncHeartbeatException.class, () -> {
            heartbeatMediator.onNewHb(outOfSyncHb);
            expectDefaultTs();
        });
    }
}